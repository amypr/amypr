...
bool Mesh::loadFromOBJFile(std::string obj_path, std::vector<ColoredVertex>& out_vertices, std::vector<uint16_t>& out_vertex_indices, vec2& out_size, std::vector<uint16_t>& out_uv_indices, std::vector<glm::vec3>& out_normals, std::vector<uint16_t>& out_normal_indices)
{
...
#pragma warning(disable:4996)
    ...
    while (1) {
        ...
        if (strcmp(lineHeader, "v") == 0) {
            if (out_vertices.size() > 0 && out_vertices.back().position.x == 0 && out_vertices.back().position.y == 0 &&
                out_vertices.back().position.z == 0 && out_vertices.back().color.x == 0 && out_vertices.back().color.y == 0 &&
                out_vertices.back().color.z == 0) {
                ColoredVertex vertex;
                int matches = fscanf(file, "%f %f %f %f %f %f\n", &vertex.position.x, &vertex.position.y, &vertex.position.z,
                    &vertex.color.x, &vertex.color.y, &vertex.color.z);
                if (matches == 3)
                    vertex.color = { 1,1,1 };
                out_vertices.back().position = vertex.position;
                out_vertices.back().color = vertex.color;
            } else {
                ColoredVertex vertex;
                int matches = fscanf(file, "%f %f %f %f %f %f\n", &vertex.position.x, &vertex.position.y, &vertex.position.z,
                    &vertex.color.x, &vertex.color.y, &vertex.color.z);
            ...
            ColoredVertex normal_vertex;
            // glm::vec3 normal_vertex_n;
            fscanf(file, "%f %f %f\n", &normal_vertex.normal.x, &normal_vertex.normal.y, &normal_vertex.normal.z);
            // fscanf(file, "%f %f %f\n", &normal_vertex_n.x, &normal_vertex_n.y, &normal_vertex_n.z);
            // out_normals.push_back(normal_vertex_n);
            normal_vertex.position = {0, 0, 0};
            normal_vertex.color = {0, 0, 0};
            out_vertices.push_back(normal_vertex);
        }
